...
function 요약HTML(label, 기계요약, 정보JSON, 기록없음){
   ...
   *  ① 그 회의체에 **기록이 0건이면 요약을 아예 안 보여준다.** 없는 것을 있는 것처럼
   *     보여주는 것이 제일 나쁘다.
   *  ② 회의체별로 따로 쓴 요약이 있으면 그것을 쓴다(`주차|임차` 같은 열쇠).
  ...
  /* 이 회의체만 추려 쓴 요약이 있으면 그것을 쓴다. 없으면 합친 요약으로 내려간다.
     어느 쪽을 쓰는지(전용)를 알아야 아래 안내 문장을 맞게 쓸 수 있다. */
  const 전용글=(S&&요약표)?S.파싱((요약표[열쇠]||{}).text):null;
  const r=전용글||((S&&요약표)?S.파싱((요약표[label]||{}).text):null);
  const 전용=!!전용글;
  /* 다시 쓴 요약은 「짚어둘 것」과 「지나온 일」이 나뉘어 온다. 한 덩어리로 늘어놓으면
     지금 벌어지는 일과 옛일이 같은 무게로 읽힌다. */
  const 구조=((요약표||{})[전용?열쇠:label]||{}).구조;
  ...
  const 짚을것=(구조&&구조.요점)?구조.요점:(r?r.요점:[]);
  const 지나온일=(구조&&구조.히스토리)?구조.히스토리:[];
  const 요점=짚을것.map(x=>`<li>${esc(x)}</li>`).join('');
  const 히스토리=지나온일.map(x=>`<li>${esc(x)}</li>`).join('');
  /* 「지나온 흐름」은 뺐다 (사용자 지적 2026-09-02: *"지나온 흐름은 사실 필요가 없어.
   * 아래 타임라인이 있기 때문에"*). 맞다 — 41줄을 늘어놓아도 바로 아래 타임라인이
   * 같은 기간을 안건 단위로 보여준다. 두 번 읽을 일이다.
   * 대신 **요점이 전 기간을 담아야 한다.** 그건 화면이 아니라 요약 글 자체를 고칠 일이라
   * 회의록 앱의 주제 흐름 요약을 다시 쓰는 쪽으로 다룬다. */
  return (r.현재상태?`<p class="tb-label">지금</p><p class="tb-now">${esc(r.현재상태)}</p>`:'')
    +(요점?`<p class="tb-label">${히스토리?'짚어둘 것':'요점'}</p><ul class="tb-points">${요점}</ul>`:'')
    +(히스토리?`<p class="tb-label">어떻게 여기까지 왔나</p><ul class="tb-history">${히스토리}</ul>`:'')
    +(기계요약?`<ul class="tb-meta">${기계요약}</ul>`:'')
    /* 회의체를 골랐을 때 (사용자 지적 2026-09-02: "각기 다르게 나와야하는데 똑같이 나와").
     * 목록과 타임라인은 제대로 걸러진다(주차 전체 83 / 입대의 48 / 임차 35).
     * 걸러지지 않는 것은 이 **요약**인데, 두 회의체를 합쳐 쓴 글이라 그렇다.
...
function 요약채우기(label){
  ...
  S.불러오기(window.fetch.bind(window)).then(function(m){
    ...
    if(자리) 자리.innerHTML=요약HTML(label, 자리.dataset.machine||'', 자리.dataset.body||'', 자리.dataset.none==='1');
  }).catch(function(){});
}
/* 타임라인을 연도별로 접는다 (5.5d).
 *
