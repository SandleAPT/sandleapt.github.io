(function(){
  ...
  window.SandleAdminViews.visibility=function(root,store){
    ...
    items.forEach(x=>{const v=policy?policy.normalizeVisibility(x.visibility):'private';counts[v]=(counts[v]||0)+1;});
    const card=item=>{const visibility=policy?policy.normalizeVisibility(item.visibility):'private';return `<article class="aw-review-card" data-item="${U().esc(item.id)}"><div class="aw-review-top"><div>${U().sampleBadge(item)}${U().visibilityBadge(visibility)}</div><button class="aw-link-btn" data-preview>미리보기</button></div><h3>${U().esc(item.title)}</h3><p>${U().esc(item.documentType)} · ${U().esc(item.date||'-')}</p><div class="aw-field-row" style="margin-top:10px"><label class="aw-field"><span>공개 등급 직접 바꿔보기</span><select data-visibility><option value="public">public · 인터넷 전체 공개</option><option value="resident">resident · 입주민 인증 후 공개</option><option value="private">private · 관리자 내부</option></select></label><div class="aw-rule-box" data-result style="margin-top:0"></div></div></article>`;};
    ...
    root.querySelectorAll('.aw-review-card').forEach(el=>{
      ...
      const paint=()=>{
        ...
        const target=policy?policy.targetFor(v):{publicBundle:false,authentication:'admin_required'};
        result.innerHTML=target.publicBundle?'<b>공개 projection 생성</b><span>허용 필드와 공개 원본 참조만 공개 번들에 포함</span>':v==='resident'?'<b>공개 빌드 제외</b><span>입주민 인증 영역이 준비될 때까지 공개 검색·본문에서 제외</span>':'<b>모든 공개 빌드 제외</b><span>관리자 내부 자료로만 유지</span>';
      };
      paint();
