  ...
  window.SandleAdminViews.relations=function(root,store){
    ...
    root.querySelectorAll('.aw-relation-card').forEach(el=>{
      ...
      const showHelp=()=>{
        ...
        // 조사(을/를·이/가)가 앞말 받침에 따라 달라져 문장으로 만들면 어색해진다.
        // 화살표에 관계를 얹는 형태로 두면 어떤 제목이 와도 자연스럽게 읽힌다.
        const 상대=(target.value||'').trim();
        sentence.textContent='저장하면 이렇게 남는다 —  「'+item.title+'」 ─['+L().typeShort(type.value)
          +']→ 「'+(상대||'연결 후보를 먼저 적어줘')+'」   (근거: '+L().evidenceShort(evidence.value)+')';
      };
      showHelp();
