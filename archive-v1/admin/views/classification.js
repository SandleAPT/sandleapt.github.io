(function(){
  ...
  window.SandleAdminViews.classification=function(root,store){
    const all=store.getState().items.filter(x=>!x.published);
    const items=all.filter(x=>!x.classificationApproved);
    const active=items.filter(x=>!x.classificationHeld);
    const held=items.filter(x=>x.classificationHeld);
    // 자동으로 정해져 사람 손을 거치지 않은 것들. 접어두되 언제든 펼쳐 볼 수 있다.
    const auto=all.filter(x=>x.autoAssigned);
    ...
    root.querySelectorAll('[data-reopen]').forEach(b=>b.onclick=()=>{store.reopenClassification(b.dataset.reopen);U().toast('검토 목록으로 꺼냈어. 고친 뒤 승인하면 돼.');});
  };
})();