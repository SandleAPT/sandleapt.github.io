(function(){
  ...
  function render(root,store){
    ...
    root.innerHTML=`<section class="aw-page-head"><div><p class="admin-kicker">STAGE 3 · 회의록 → ARCHIVE</p><h1>이미 만든 회의록을 다시 쓰지 않고 정리</h1><p>현재 `/minutes/`의 실제 정적 회의록 데이터를 읽어서 회의 전체와 안건을 Archive 구조로 나눠 보여줘. 실제 회의 213건과 안건 1,125건 변환 검증을 통과했고 기존 작성·저장·PDF는 건드리지 않아.</p></div><button class="aw-ghost" data-go="dashboard">관리 홈</button></section><section class="s3-stepbar"><span class="done"><b>3.1</b>현행 구조 확인</span><i>→</i><span class="done"><b>3.2</b>필드 매핑</span><i>→</i><span class="done"><b>3.3~3.5</b>실제 회의 변환</span><i>→</i><span class="done"><b>3.6~3.7</b>검증 완료</span></section>${vm.error?`<div class="s3-error">${esc(vm.error)}</div>`:''}<div class="s3-workspace"><aside class="aw-panel s3-picker"><div class="s3-picker-controls"><label><span>연도</span><select data-year>${years.map(y=>`<option value="${esc(y.year)}" ${String(y.year)===vm.year?'selected':''}>${esc(y.year)}년 · ${esc(y.count)}건</option>`).join('')}</select></label><label><span>회의체</span><select data-filter><option value="all">전체</option><option value="입대의" ${vm.filter==='입대의'?'selected':''}>입주자대표회의</option><option value="임차" ${vm.filter==='임차'?'selected':''}>임차인대표회의</option></select></label></div><label class="s3-search"><span>⌕</span><input data-query placeholder="회의명 검색" value="${esc(vm.query)}"></label><div class="s3-list">${vm.loading?'<div class="s3-loading">회의록 불러오는 중…</div>':listHtml()}</div></aside><main class="s3-detail">${selectedHtml()}</main></div>`;
