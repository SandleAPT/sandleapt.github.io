(function(){
  ...
  window.SandleAdminViews.storagePolicy=function(root,store){
    ...
    root.innerHTML=`<section class="aw-page-head"><div><p class="admin-kicker">STAGE 4 · 저장·권한</p><h1>공개 파일에 넣기 전에 먼저 분리</h1><p>화면에서 감추는 방식이 아니라 공개 번들 자체를 허용 필드로 새로 만들고, resident/private 자료는 GitHub Pages 파일에서 제외해.</p></div><button class="aw-ghost" data-go="dashboard">현재 작업 홈</button></section>
      <section class="s4-store-grid"><article><span>공개 포털</span><h2>GitHub Pages</h2><p>화면 코드, 공개 메타데이터, 검색 인덱스, taxonomy와 공개 원본 참조만 둬.</p></article><article><span>회의 원본</span><h2>minutes</h2><p>기존 회의 작성·저장·PDF 흐름을 유지하고 Archive는 공개 데이터를 읽기만 해.</p></article><article><span>파일 원본</span><h2>⑨ 비공개 저장소</h2><p>PDF·사진 같은 민감 원본은 회의록 앱 ⑨ 저장소에 둬. 읽기·쓰기를 서버가 비밀번호로 확인하고, 공개 포털에는 아무것도 싣지 않아. 항목당 약 8MB가 한계라 더 큰 자료가 생기면 그때 드라이브로 옮겨.</p></article></section>
