...
(function (root, factory) {
  ...
  return {
    ...
    run: function (ctx) {
      ...
      assert.equal(고친줄.주제들.join(','), '선거·임원', '고친 주제가 반영된다');
      assert.equal(고친줄.자동주제, '기타', '자동 판정이 무엇이었는지도 남긴다');
      assert.equal(M.sortRows(고침)[0].id, 'd', '고친 것이 맨 위로');

      /* ── 회의록에 이미 붙은 수동 태그를 절대 덮지 않는다 ────────────────
       * 실제 데이터에 250건이 이미 수동 태그를 갖고 있었다. 이걸 무시하면
       * 사람이 정해둔 값을 자동 판정으로 되돌려 놓게 된다.
       * 태그는 **여러 개**다(예: 관리규약·잡수입·예산·전기·설비).
       * 하나만 고르게 만들면 저장할 때 나머지를 지운다.
       */
      var 수동안건 = [{ id: 'm', title: '주차장 차단기', note: '',
        tags: ['관리규약', '잡수입·예산', '전기·설비'], 날짜: '2020-01-01' }];
      var 수동줄 = M.buildRows(수동안건, 판정, {})[0];
      assert.equal(수동줄.상태, 'manual', '수동 태그가 있으면 manual');
      assert.equal(수동줄.주제들.length, 3, '태그 세 개가 모두 남는다');
      assert.equal(수동줄.주제들.join(','), '관리규약,잡수입·예산,전기·설비', '순서까지 그대로');
      assert.equal(수동줄.자동주제, '주차', '자동 판정은 참고로만 남긴다');
      assert.equal(수동줄.rank > 3, true, '이미 정해둔 것은 맨 아래 — 다시 볼 일이 없다');

      // 이 화면에서 고치면 수동 태그보다 우선한다.
      var 수동덮기 = M.buildRows(수동안건, 판정, { m: ['주차', '하자·소송'] })[0];
      assert.equal(수동덮기.상태, 'fixed', '방금 고친 것이 이긴다');
      assert.equal(수동덮기.주제들.join(','), '주차,하자·소송', '고친 값이 쓰인다');
      assert.equal(수동덮기.수동태그.join(','), '관리규약,잡수입·예산,전기·설비', '원래 태그가 무엇이었는지 남는다');

      // 여러 주제 중 하나만 맞아도 걸러진다.
      assert.equal(M.filterRows([수동줄], { 주제: '전기·설비' }).length, 1, '대표가 아닌 주제로도 걸린다');
      // 한 안건이 여러 주제에 걸리면 각각에 센다.
      var 여러세기 = M.countByTopic([수동줄]);
      assert.equal(여러세기.length, 3, '세 주제 모두 센다');
      ...
      assert.equal(M.filterRows(줄, { 주제: '주차' }).length, 2, '대표 주제로 거르기');
