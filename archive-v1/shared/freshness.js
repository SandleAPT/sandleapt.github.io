...
(function (root, factory) {
  ...
  function 문구(r) {
    if (!r) return '';
    if (!r.뒤처짐) return '클라우드와 같습니다. 빠진 회의가 없습니다.';
    var n = r.빠진것.length;
    return '클라우드에 ' + n + '건이 더 있습니다. 사본에는 아직 반영되지 않았고, 아래에서 바로 열어볼 수 있습니다.';
  }
  ...
  function 멈춤안내(지난날) {
    if (!(지난날 >= 3)) return '';
    return '사본은 하루 한 번 다시 만들어지는데 ' + 지난날 + '일째 그대로입니다. ' +
      '만드는 작업(minutes 저장소의 자동 실행)이 실패하고 있을 가능성이 높습니다. ' +
      '그동안에도 회의록 앱은 클라우드를 직접 읽으므로 최신입니다.';
  }
