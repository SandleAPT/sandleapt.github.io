...
(function (root, factory) {
  ...
  function 요약(rows) {
    ...
    var 몰림 = 실패몰림(r);
    var 문구 = [];
    if (!r.length) 문구.push('기록이 아직 없습니다. 서버에 기록 기능을 올린 뒤부터 쌓입니다.');
    else {
      문구.push('최근 ' + r.length + '건 — 수정용 ' + 셈.수정용 + ' · 열람용 ' + 셈.열람용 +
        ' · 틀림 ' + 셈.실패 + ' · 권한 없이 시도 ' + 셈.거부 + '. 기기 ' + Object.keys(기기).length + '대.');
      if (몰림.length) {
        문구.push('짧은 시간에 실패가 몰린 기기가 ' + 몰림.length + '대 있습니다. 누군가 비밀번호를 찍어보고 있을 수 있습니다.');
      } else if (셈.실패 || 셈.거부) {
        문구.push('실패가 있지만 몰려 있지는 않습니다. 대개 오타입니다.');
      } else {
        문구.push('이상해 보이는 것은 없습니다.');
      }
      문구.push('기기 표시는 그 기기가 스스로 밝힌 값이라 신원으로 믿을 수 없습니다. 서버는 접속한 곳을 알 수 없습니다.');
    }
