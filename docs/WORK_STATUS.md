...
마지막 갱신: 2026-09-02 01:40:10 KST
...
- **주제 훑어보기에서 고친 값을 어디에 저장할 것인가** (2026-09-02 발생). 지금은 그 브라우저에만 남아 다른 기기에서 안 보이고 지워지면 사라진다. 회의록 앱의 안건 수동 태그(`a.tags`)에 쓰는 것이 자연스럽지만 「원본 덮어쓰기」에 해당한다. 선택지: ① 회의록 클라우드에 직접 쓰기 ② Archive 쪽에 따로 저장하고 회의록은 건드리지 않기 ③ 지금처럼 브라우저에만 두기.
