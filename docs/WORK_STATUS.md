...
마지막 갱신: 2026-09-02 03:50:00 KST
...
**남은 것**: 회의 하나를 고치려고 그 회의 전체를 세 번 왕복한다(평상시 약 8초). 회의 224건을 전부 손대면 30분쯤이다 — 못 할 정도는 아니지만, 작은 수정에 레코드 전체를 다시 쓰는 구조 자체가 9/1 날짜 유실의 원인이었다. **해법 후보는 레코드 분리보다 서버 쪽 부분 수정**(GAS에 안건 태그만 바꾸는 action 추가)이다. 왕복이 1회로 줄고, 브라우저가 레코드 전체를 다시 만들지 않으니 다른 필드를 건드릴 여지가 사라진다.
