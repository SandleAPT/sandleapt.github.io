...
- **눌러 보고 고친 것 둘** — 이 둘은 spec으로는 안 잡히는 종류였다.
  ...
  2. **Apps Script 읽기 한 번에 24초**(실측). 회의 하나에 읽기·저장·재조회 세 번을 왕복하므로 회의당 1분 가까이 걸린다. 처음엔 코드가 멈춘 줄 알고 파고들었다. 진행 표시에 경과 시간과 예상 소요를 넣었다.
- 커밋: `03ed5c0`(모듈·버튼·spec), `b8a8684`(저장 후 반영·진행 표시)

**남은 것**: 대량 저장은 아직 느리다. 회의 20건을 고치면 20분이 걸린다. `4.6`(신선도)과 `SCALING.md` §5(안건 단위 레코드 분리)가 같이 풀어야 할 문제다.
