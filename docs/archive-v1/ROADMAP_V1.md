...
- 2.4g `done` — **훑어보기 화면에서 회의록으로 저장.** `shared/tag-writer.js`에 규칙을 박아 두고(네트워크를 주입받아 실제 저장 없이 검증), 화면에 「회의록에 저장」 버튼을 붙였다. 배포본 13/13 통과 + 실제 클라우드 저장 확인. 눌러 보고 고친 것 둘: ① 저장 뒤 다시 읽으면 옛 값이 돌아왔다 — 이 화면은 하루 한 번 갱신되는 정적 사본을 읽기 때문(`4.6`). 저장한 값을 화면에 그대로 반영하도록 바꿈 ② Apps Script 읽기 한 번에 **24초**(실측)가 걸려 회의당 1분 가까이 든다. 경과 시간과 예상 소요를 표시. (2026-09-02 03:50 KST)
