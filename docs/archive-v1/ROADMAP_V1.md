...
- 앞 단계가 `review`여도 사용자가 **실제로 써보며 보완하기로 하고 다음 단계 진행을 명시적으로 승인**하면, 확정 잠금 없이 다음 단계로 진행할 수 있다. 이 경우 앞 단계는 `review` 상태로 남긴다.
...
### 3.2 기존 회의 데이터 → Archive 필드 매핑 — `review`
