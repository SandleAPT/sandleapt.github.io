...
# 4.3b 인증·접근 판정 구현 (Claude)

- 검증·원격 반영 완료: `2026-09-01 20:48:15 KST`
- 캐시 버전: `20260901-2035`
- 사용자 결정: 2026-09-01 19:45 KST — 회의록 앱의 열람용/수정용 비밀번호 재사용(별도 회원제 없음)
- 설계: `docs/archive-v1/AUTH_V1.md`
- 운영 루트 변경: 없음 / minutes 원본 변경: 없음 / 공개 범위 변경: 없음

## 구현

기능을 두 모듈로 나눴다. 세션 관리와 판정 규칙을 섞으면 화면마다 다른 조건문이 생기기 때문이다.

- `archive-v1/shared/auth-session.js` — 서버 검증, 세션 저장·만료, 현재 role 조회
- `archive-v1/shared/access-control.js` — role → 볼 수 있는 공개등급 판정(단일 출처)

| role | 볼 수 있는 등급 | 관리 화면 |
|---|---|---|
| `''`(비로그인) | public | 불가 |
| `view`(열람용) | public, resident | 불가 |
| `edit`(수정용) | public, resident, private | 가능 |

모르는 role·모르는 등급·응답 오류는 전부 가장 엄격한 쪽으로 떨어진다(fail-closed).

## 검증 결과 — 실제 배포본

`https://sandleapt.github.io/archive-v1/tests/browser/` → **9/9 통과, 실패 0**

Stage 3 3종 + Stage 4 6종(`source-reference`, `visibility`, `publish-guard`, `admin-integration`, **`access-control`**, **`auth-session`**), 실제 데이터 213회의/1,125안건.

## 검증이 잡아낸 결함 — 회수된 비밀번호가 계속 통과함

`stage4-auth-session`이 첫 실행에서 실패했다.

```text
❌ stage4-auth-session — 서버가 거부하면 role 없음 — 기대 "", 실제 "view"
```

원인은 `currentRole()`이 세션에 캐시된 role을 **무기한** 신뢰한 것이었다. 이 상태면 비밀번호를 교체하거나 회수해도 이미 로그인한 브라우저는 **24시간 내내 통과**한다. 화면만 보고는 알 수 없는 종류의 결함이라, 자동 검증이 없었으면 그대로 배포됐을 것이다.

**수정**: 재확인 주기 5분을 도입했다.

- 5분 이내 → 캐시 사용(불필요한 서버 왕복 없음)
- 5분 초과 → 저장된 키로 재검증, 거부되면 키 삭제 후 `role=''`
- `currentRole(true)` → 캐시 무시하고 즉시 재검증(민감 동작 직전용)

회수 반영이 최대 24시간에서 **최대 5분**으로 줄었다.

## spec이 실제 로그인 상태를 건드리지 않는지 확인

`stage4-auth-session`은 `localStorage`의 인증 키를 조작한다. 같은 origin이라 **회의록 앱의 로그인까지 풀릴 수 있어**, spec이 원래 값을 백업했다가 `finally`에서 되돌리도록 했다. 실행 후 키 상태가 실행 전과 같은 것을 확인했다.

## 이번 단계에서 의도적으로 하지 않은 것

- resident/private **원본을 실제로 내려주는 서버** — 외부 저장소 결정(`4.1`)과 묶여 있어 별도 단계
- 시도 횟수 제한·감사 로그 — `4.3c`
- 화면 연결(로그인 UI, 등급별 목록 표시) — `4.4`에서 사용자 검토와 함께

인증 모듈이 생겼다고 해서 공개 번들 정책은 바뀌지 않는다. 정적 번들에는 여전히 `public`만 들어간다.
