(function(){
  ...
  window.ProposalPrint={create:function(api){
    ...
    function selected(){return Array.from(list.querySelectorAll('input:checked')).map(function(input){return rows[Number(input.value)];});}
    ...
    dialog.querySelector('[data-all]').onclick=function(){list.querySelectorAll('input').forEach(function(el){el.checked=true;});count();};
    ...
    return {open:async function(){
      ...
      try{
        ...
        rows.forEach(function(doc,i){var label=document.createElement('label');label.className='print-choice';var input=document.createElement('input');input.type='checkbox';input.value=String(i);var span=document.createElement('span');span.textContent=api.date(doc.date)+' · '+(doc.docType==='report'?'보고':'의결')+' '+api.number(doc)+'호 · '+doc.title;label.append(input,span);list.appendChild(label);});
