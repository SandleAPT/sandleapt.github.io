(function(root){
  ...
  function create(options){
    ...
    async function request(key){
      ...
      try{
        return await Promise.race([
          (async function(){var response=await options.fetch(options.url,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'verify',adminKey:key,token:options.token}),signal:controller.signal});
    ...
    async function login(key,trust){
      ...
      if(!next){clear();verifiedKey='';throw new Error('비밀번호가 올바르지 않습니다.');}
      ...
      verifiedKey=key;verifiedAt=Date.now();apply(next);return true;
    }
    async function restore(force){
      ...
      try{var next=await request(key);if(ticket!==sequence||session()!==key)return;
        verifiedKey=key;verifiedAt=Date.now();if(!next)clear();apply(next);
