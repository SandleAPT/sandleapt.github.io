...
(async()=>{
  ...
  const url='https://script.google.com/macros/s/test_private/exec',fresh=fixture();
  fresh.setReply({ok:true,role:'edit',privateStoreUrl:url});await fresh.auth.login('test-edit',false);
  assert.equal(fresh.storage.getItem('sandle_private_url'),url);
  const denied=fixture();denied.setReply({ok:true,role:'view',privateStoreUrl:url});await denied.auth.login('test-view',false);
  assert(!denied.storage.getItem('sandle_private_url'));
  denied.setReply({ok:true,role:'edit',privateStoreUrl:'https://evil.test/exec'});await denied.auth.login('test-edit',false);assert(!denied.storage.getItem('sandle_private_url'));
