...
회의자료의 `인쇄 · PDF`는 최근 회의일순 목록에서 체크한 저장 자료만 표지·본문 순서로 출력합니다. 같은 날짜는 의결안건·보고사항의 번호순이며, 현재 작성 중인 내용은 `현재 작성본 인쇄`로 출력합니다. 자료를 체크하면 첨부별 출력 선택이 나타나며 PDF·JPG·PNG는 해당 본문 뒤에 출력됩니다. HWP는 다운로드하여 한글에서 별도 인쇄합니다. 클라우드는 수정 로그인 시 자동 연결되고 `목록 새로고침`으로 다시 조회합니다.
