...
근거규정·기타는 비어 있으면 자동으로 숨기며, 각 `제목 표시`를 끄면 ‘가. 근거규정 / 나. 기타’ 제목만 숨기고 내용은 유지합니다. `내용 숨기기`는 해당 항목 전체를 출력에서 숨기며 입력은 보존합니다. 기존 showBasis/showRefs는 내용 표시 의미를 유지하고, 새 showBasisTitle/showRefsTitle은 생략 시 true로 처리합니다. 옵션은 문서 저장에 포함됩니다. 긴 본문은 다음 쪽으로 이어서 출력합니다.

본문이 여러 쪽으로 이어지면 회의명·상단 구분선과 하단 안건번호·안건명을 각 쪽에 반복합니다. 일괄 인쇄에서도 문서마다 해당 안건명이 적용됩니다.
